document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. Handle Video Processing Request
    ...
    // 2. Handle Chat Message Submission
    chatForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (res.ok && data.status === "success") {
                // Safeguard against raw empty space strings
    ...
    // 3. Document Node Appender with Basic Markdown Conversion
    function appendMessage(className, text) {
        const div = document.createElement('div');
        div.className = `message ${className}`;
        
        if (className === "ai-msg") {
            // Parses markdown markdown syntax safely for the frontend interface
        ...
        chatMessages.scrollTop = chatMessages.scrollHeight; // Auto scrolls stream window
    }
});